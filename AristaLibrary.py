...
def connect_to(node, proto="https", port="443", username="admin",
               passwd="admin"):
    """Try to connect to an EOS node over JSONRPC"""

    url = "%s://%s:%s@%s:%s/command-api" % \
          (proto, username, passwd, node, port)
    eapi = jsonrpclib.Server(url)
    ...
    for rpm in rpms:
        if rpm['name'] == name:
            ...
            if rpm['version'] != version:
                success = False
                log += "\nVersion is NOT a match: %s != %s" % \
                       (rpm['version'], version)

            if rpm['available'] != available:
                success = False
                log += "\nAvailable is NOT a match: %s != %s" % \
                       (rpm['available'], available)

            if rpm['installed'] != installed:
                success = False
                log += "\nInstalled is NOT a match: %s != %s" % \
                       (rpm['installed'], installed)
    if success:
        return True
    elif extFound:
        raise AssertionError("Extension found but with params:%s\n" % log)
    else:
        raise AssertionError(
            "Extension not found in returned in eAPI command\n"
            "RawOutput:\n%s" % result)
